...
if ($_GET['do'] == 'new') {
    $query = "INSERT INTO sandbox (name, time, comment, url, img_url, type, pr) VALUES ('$name', '$time', '$comment', '$url', '$img_url', '$type', '$pr')";
    $sql = mysql_query($query) or die(mysql_error());
}
if ($_GET['do'] == 'edit') {
    $newsid = mysql_real_escape_string($_GET['id']);
    $query = "UPDATE `index` SET `name`='$name', `comment`='$comment', `url`='$url', `img_url`='$img_url', `pr`='$pr' WHERE id='$prid'";
    $sql = mysql_query($query) or die(mysql_error());
}
if ($_GET['do'] == 'delete') {
    $query = "DELETE FROM `sandbox` WHERE `id`='$prid'";
    $sql = mysql_query($query) or die(mysql_error());
}
